// scripts/build_maifatto_it_all.mjs (ROBUST • FOOD-RICH • WOW • Anti-Overpass)
// Output: public/data/mai_fatto/mai_fatto_it_all.json
//
// Fix principali:
// - Multi-endpoint Overpass (fallback automatico)
// - Retry con exponential backoff su 429/504/timeouts
// - Query spezzata in 2 passate per tile: (A) WOW+FAMILY  (B) FOOD
// - Salva comunque un dataset "parziale" se alcuni tile falliscono (niente più 404)
//
...
const SLEEP_MS_BASE = Number(process.env.SLEEP_MS || 1800); // più alto
const MAX_TILES = process.env.MAX_TILES ? Number(process.env.MAX_TILES) : null;
const GRID = {
  cols: Number(process.env.GRID_COLS || 8), // default più leggero
  rows: Number(process.env.GRID_ROWS || 8),
};

// Italia bbox approx (W,S,E,N)
const IT_BBOX = { w: 6.5, s: 36.0, e: 18.7, n: 47.2 };

// vogliamo tanti posti, ma prima facciamo funzionare (poi alziamo)
const TARGET_IDEAS = Number(process.env.TARGET_IDEAS || 8000);
  ...
  for (let attempt = 1; attempt <= 6; attempt++){
    for (const ep of ENDPOINTS){
      try{
        ...
        // alcuni 504 tornano HTML (come vedi tu)
        if (!r.ok){
          const txt = await r.text().catch(() => "");
          const msg = `HTTP ${r.status} (${ep}) ${txt.slice(0,120)}`;
          // 429/504 => retry
          if (r.status === 429 || r.status === 504 || r.status === 502 || r.status === 503){
        ...
        if (!ct.includes("application/json")){
          const txt = await r.text().catch(() => "");
          // se è HTML, trattalo come overload e retry
        ...
        lastErr = e;
        // continua con altri endpoint / retry
      }
    }
...
async function main(){
  console.log("BUILD MAI FATTO IT (ROBUST) — endpoints:", ENDPOINTS);
  ...
  const out = {
    ...
    count: ideas.length,
    area: "Italia (tutto il territorio) — WOW + Food ricchissimo (robust build)",
    stats: { tiles_total: tilesToRun.length, tiles_ok: okTiles, tiles_failed: failTiles, endpoints: ENDPOINTS },
    ideas
  };
