...
st.id = "jamo-mini-css";
st.textContent =   .moreBtn{width:100%; border:1px solid rgba(255,255,255,.14); background:rgba(255,255,255,.04); color:#fff; border-radius:16px; padding:12px; font-weight:950; cursor:pointer;}   .optBtn{width:100%; text-align:left; border:1px solid rgba(255,255,255,.10); background:rgba(255,255,255,.03); color:#fff; border-radius:16px; padding:12px; cursor:pointer;}   .optBtn:active{transform:scale(.99)}   .optList{display:flex; flex-direction:column; gap:10px;}   .pill{display:inline-flex; gap:8px; align-items:center; padding:7px 10px; border-radius:999px; border:1px solid rgba(255,255,255,.14); background:rgba(0,0,0,.25); font-weight:850; font-size:13px;}  ;
...
function familyBoost(place, categoryUI) {
...
function cantineBoost(place, categoryUI) {
const category = canonicalCategory(categoryUI);
...
for (const raw of pool) {  
  ...
  if (km < (canonicalCategory(categoryUI) === "family" ? 1.2 : 1.6)) continue;  
  ...
  s += seasonAdjust(p);  
  s += cantineBoost(p, categoryUI);  

  if (!ignoreRotation) s -= rotationPenalty(pid, recentSet);  

  if (canonicalCategory(categoryUI) === "relax") {  
    const t = tagsStr(p);  
...
if (!area) return;
area.innerHTML =   <div class="card" style="box-shadow:none; border-color:rgba(255,180,80,.35); background:rgba(255,180,80,.06);">   <div style="font-weight:950; font-size:18px;">🔎 Sto cercando…</div>   <div class="small muted" style="margin-top:8px; line-height:1.4;">${escapeHtml(msg)}</div>   </div>  ;
}
