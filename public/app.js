...
showStatus(
"err",
"Eventi non disponibili: manca events.js oppure non è stato caricato."
);
const area = $("resultArea");
if (area) {
area.innerHTML =   <div class="card clickSafe" style="box-shadow:none; border-color:rgba(255,90,90,.40); background:rgba(255,90,90,.10);">   <div style="font-weight:950; font-size:18px;">❌ Eventi non disponibili</div>   <div class="small muted" style="margin-top:8px; line-height:1.45;">   Controlla che in <b>index.html</b> ci sia <b>/events.js</b> prima di <b>/app.js</b>.   </div>   </div>  ;
}
...
if (
!j.result ||
!Number.isFinite(Number(j.result.lat)) ||
!Number.isFinite(Number(j.result.lon))
) {
...
return true;
if (hasAny(t, ["man_made=survey_point", "power=", "telecom=", "pipeline=", "place=locality"]))
return true;
if (hasAny(n, ["parcheggio", "stazione", "fermata", "svincolo", "uscita", "cabina", "impianto", "linea", "km "]))
return true;
return false;
}
...
(nm.includes("terme") || nm.includes("spa") || nm.includes("thermal") || nm.includes("wellness"));
return spaTags || looksWellnessByName(place) || poolSpaLike;
}

function isThemePark(place) {
return tagsStr(place).includes("tourism=theme_park");
}
function isWaterPark(place) {
return tagsStr(place).includes("leisure=water_park");
}
...
if (t.includes("tourism=alpine_hut") || t.includes("amenity=shelter") || t.includes("building=hut"))  
  return true;  
...
if (hasAny(n, ["cantina", "winery", "vino", "vini", "enoteca", "degustaz", "wine tasting", "wine tour"]))
return true;
return false;
}
...
const lodging =  
  ...
  t.includes("tourism=hostel") ||  
  t.includes("tourism=guest_house") ||  
  t.includes("tourism=apartment") ||  
  t.includes("tourism=camp_site") ||  
  t.includes("tourism=caravan_site") ||  
  t.includes("tourism=chalet") ||  
  t.includes("tourism=motel");  

const food =  
  t.includes("amenity=restaurant") ||  
  t.includes("amenity=fast_food") ||  
  t.includes("amenity=cafe") ||  
  t.includes("amenity=bar") ||  
  t.includes("amenity=pub") ||  
...
if (quality && hasAny(n, ["castell", "abbazi", "duomo", "cattedral", "museo", "gole", "cascat", "parco", "riserva", "oasi", "lago", "forte", "rocca"])) {  
  return true;  
}  
...
if (category === "storia") return "Storia • castelli/musei/attrazioni (ver
