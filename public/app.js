/* Jamo — app.js v22.3 (CLEAN • TOURISTIC • MONETIZZABILE • TAP-SAFE)
 * ✅ Offline-first • ✅ NO GPS • ✅ Region-first IT
 * ✅ Mare fix: solo mare vero + widen se non trova vicino
 * ✅ Mai fatto: usa events.js (window.JAMO_EVENTS.run)
 * ✅ Tap-safe + anti-irrilevanti
 */
...
(() => {
  ...
  // ✅ categoria canonica: Eventi/UI "mai fatto" => "eventi"
  function canonicalCategory(cat) {
    const c = String(cat || "").toLowerCase().trim();
    if (c === "trekking" || c === "hiking") return "hiking";
    if (c === "mai_fatto" || c === "maifatto" || c === "mai fatto") return "eventi";
...
function bindResultAreaDelegation() {
  ...
  function widenMinutesSteps(m, categoryUI) {
    ...
    // ✅ Mare: prova a trovare vicino, ma se non c’è ALLARGA parecchio (senza cambiare categoria)
    const muls =
      category === "family" ? [1.15, 1.30, 1.50] :
      category === "mare"   ? [1.20, 1.45, 1.75] :
    ...
    // “ancora più largo” finale (utile soprattutto per mare/family)
  ...
  async function runSearch({ silent = false, forbidPid = null } = {}) {
    ...
    try {
      ...
      // ✅ MAI FATTO / EVENTI (bridge a events.js)
      if (cat === "eventi") {
      ...
      for (const mins of steps) {
        ...
        // 2) se region ha qualcosa, unisci anche altro
